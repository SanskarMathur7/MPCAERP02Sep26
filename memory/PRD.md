...
> Started: Jan 2026 · Last update: Feb 2026 — Squad Selection Iteration A

## Recent Changelog
- **Feb 2026 — Sprint Squad-Iter-A · Squad Selection Cleanup (7 of 12 tickets)**: User uploaded a 12-ticket batch for the squad module. Iteration A ships the bug fixes + workflow consolidation; Iteration B (age filter · per-player review · PDF overhaul) is next. **MPCA-125 · Two Squad Tabs Confusion**: TournamentDetail had TWO squad entry points — `box-squads` (multi-body panel) AND `box-squad` (direct-nav Squad Selection). Deleted the second box; the multi-body `Squads` box is now the single entry point. **MPCA-154 · No player-add from Tournament Detail**: TournamentDetail had a full inline "Participating Teams / Squads" grid with an "Add Player" button, a New-Squad dialog, and an Add-Player dialog — MPCA could add players directly bypassing the workflow. Deleted the entire inline section + both modal dialogs; every squad interaction now lives on `/squads/{sid}`. Also removed the dead state/handlers (`newSquad`, `addPlayer`, `openAddPlayer`, `handleCreateSquad`, `handleAddPlayer`, `handleRemovePlayer`) and the associated imports (`createSquad`, `addPlayerToSquad`, `removePlayerFromSquad`, `fetchPlayers`, `Plus`, `X`, `Crown`, `AlertTriangle`, `BadgeCheck`… kept only what the acceptance banner needs). **MPCA-131 · Post-Approval Edit Lock**: Backend already returned HTTP 409 on add/remove-player when `submission_status='Approved'`. Frontend `canEdit` in SquadDetail was `isMPCA` alone (always true for MPCA) → the +Pick/+Cap/X buttons remained visible even when locked, causing confusion. Fixed to `(isMPCA && status !== 'Approved')` so the buttons disappear post-Approval; only the Reopen button remains for MPCA. **MPCA-130 · Finalize XV Removed**: Comment says the button was removed but `canFinalize` variable was still declared. Also `handleReview` had a dead `'finalize'` branch and a `??` operator precedence bug (`x ?? ""` in a ternary consequent evaluated wrong when user cancelled). Cleaned up: `handleReview` now only handles 'approve'/'reject'; `canFinalize` deleted. **MPCA-106 · Support-Staff Only in Squad**: `OFFICIAL_SLOTS` in SquadDetail had 8 slots (manager, coach, trainer, physio, umpire_1, umpire_2, scorer, referee). User said umpires/scorer/referee belong in the Match Officials module — squad should only carry support staff. Reduced to 4 slots (Manager, Head Coach, Trainer, Physio). Removed the `isMatchOfficial` dropdown branch with the officials-DB fetch — all remaining slots are free-text. Section header renamed "Match Officials" → "Support Staff". Backend `SquadOfficialsPatch` keeps the umpire/scorer/referee keys optional for back-compat (legacy squads' values still readable) but the UI no longer sends them. **MPCA-126 · Crash on Squad Selection**: SquadDetail's player-pool row crashed when a legacy player had `full_name=null` — `p.full_name.split(" ")` threw. Added null-safe fallbacks: initials render as `(p.full_name || "?").split(" ").filter(Boolean)...`, list line falls back to `(unnamed player)` and `—`, search filter uses `(p.full_name || "").toLowerCase()`. **MPCA-141 · Squad Approval/Rejection Notifications**: `review_squad_by_mpca` now inserts a `db.notifications` row for the squad-owning body's office bearers when MPCA approves or rejects. Recipient role ids cover all common cases: `secretary`, `president`, `division-secretary`, `district-secretary`. Title/message/severity/kind vary by outcome (`Squad approved` / green · success vs `Squad rejected · please revise` / warning). Testing agent found the `Notification` Pydantic Literals were too narrow (`kind` missing `squad_review`, `severity` missing `success`) — the exception was previously swallowed by a bare `except`. Widened both Literals in `models.py`; replaced the bare `except pass` with structured `logging.warning` so future schema drifts surface. **Testing** (`iteration_88.json`): 5/5 pytest GREEN in `test_mpca_squad_iter_a.py`, 16/16 UI assertions green. Remaining squad work in Iteration B: MPCA-136 age filter, MPCA-137 age-field sync, MPCA-140 per-player approve/reject, MPCA-127 signable letterhead PDF, MPCA-142 per-player review PDF.
