...
export const WarmKpiHero = ({ label, value, accent = "emerald", trend, suffix, testid }) => {
    ...
    return (
        ...
        >
            ...
            <div className="mt-2 text-[40px] leading-none tracking-tight" style={{ fontFamily: DL.fontDisplay, fontWeight: 800, color }}>
                {value}
                {suffix && <span className="ml-1 text-[20px]" style={{ color: DL.muted }}>{suffix}</span>}
            </div>
            {trend && (
                <div className="mt-2 text-[11px] font-bold" style={{ fontFamily: DL.fontMono, color: trend.dir === "up" ? DL.emerald : DL.danger }}>
